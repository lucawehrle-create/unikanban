...
export async function regenerateRecurring(course: Course, semester: Semester): Promise<number> {
  ...
  return db.transaction('rw', db.tasks, async () => {
    ...
    // Startwochen-Änderungen (die die Woche verschieben). Fällt auf die Woche
    // zurück, wenn der Titel keine Nummer trägt (z. B. manuell umbenannt).
    const indexOf = (t: { title: string; order: number }) => {
      const m = /(\d+)\s*$/.exec(t.title)
      return m ? m[1] : `w${t.order}`
    }
    const key = (t: { recurringId?: string; title: string; order: number }) =>
