...
export async function regenerateRecurring(course: Course, semester: Semester): Promise<number> {
  ...
  return db.transaction('rw', db.tasks, async () => {
    ...
    // Erhalten bleiben: erledigte/bewertete Auto-Aufgaben UND offene, die der
    // Nutzer bereits angefasst hat (userEdited) – sonst gingen seine Änderungen
    // (Titel, Frist, Notizen, Punkte …) beim Neugenerieren verloren.
    const keep = existing.filter(
      (t) => t.autoGenerated && (t.status !== 'offen' || t.userEdited),
    )
