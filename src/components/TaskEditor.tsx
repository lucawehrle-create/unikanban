    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button
            onClick={close}
            className="rounded-lg bg-slate-800 px-4 py-1.5 text-sm font-medium text-white hover:bg-slate-700 dark:bg-slate-100 dark:text-slate-900"
          >
            Fertig
          </button>
        </>
    ...
    >
      <div className="space-y-4">
        <input
          ...
          className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium outline-none focus:border-sky-400 dark:border-slate-700 dark:bg-slate-800"
        />
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500">Kurs</span>
            <select
              value={task.courseId ?? ''}
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-medium text-slate-500">Fällig</span>
          <input
            type="datetime-local"
        ...
        {(task.type === 'uebung' || task.points) && (
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-slate-500">Punkte</span>
            <input
              type="number"
              ...
              className="w-20 rounded-lg border border-slate-200 px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-800"
            />
        ...
        {task.phases.length > 0 && (
          <div>
            <span className="mb-1.5 block text-xs font-medium text-slate-500">Schritte</span>
            <div className="space-y-1">
              {task.phases.map((p, i) => (
                ...
                >
                  ...
                  />
                  <span className={cn(p.done && 'text-slate-400 line-through')}>{p.label}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-medium text-slate-500">Notizen</span>
          <textarea
            defaultValue={task.notes ?? ''}
            onBlur={(e) => patch({ notes: e.target.value || undefined })}
            rows={2}
            className="w-full resize-none rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 dark:border-slate-700 dark:bg-slate-800"
          />
        </label>
      </div>
    </Modal>
